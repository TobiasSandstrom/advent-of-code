const day5 = (input) => {
    ...
    for (const instruction of execution) {
        ...
        for (let i = 0; i < Number(instruction[1]); i++) {
            blocksToMove.push(towers[Number(instruction[3]) - 1][towers[Number(instruction[3]) - 1].length - 1]);
            towers[Number(instruction[3]) - 1].pop();
        }
        towers[Number(instruction[5]) - 1] = towers[Number(instruction[5]) - 1].concat(blocksToMove.reverse());
        console.log(towers);
    }
    let res = "";
    for (let i = 0; i < towers.length; i++) {
        if (towers[i][towers[i].length - 1] == undefined)
            continue;
        res = `${res}${towers[i][towers[i].length - 1]}`;
    }
    console.log("Day 5, Part 2 result ---->", res);
};
...
move 9 from 6 to 3`;
day5(input);
//# sourceMappingURL=day_5_part_2.js.map